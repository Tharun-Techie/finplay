...
export default function GuessTheStockPage() {
  ...
  const puzzleId = `daily-${today}-guess-stock`;
  ...
  useEffect(() => {
    fetch(`/api/games/guess-stock/${puzzleId}`, { cache: "no-store" })
      .then((r) => {
  ...
  return (
    <Stack spacing={2} sx={{ maxWidth: 720, mx: "auto" }}>
      ...
      {loadError && (
        <Alert severity="error">Couldn&apos;t load today&apos;s puzzle: {loadError}</Alert>
      )}
