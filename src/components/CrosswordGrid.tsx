...
import type { PublicCrossword } from "@/data/puzzles";

export interface CrosswordGridProps {
  data: PublicCrossword;
  puzzleId: string;
}
...
export default function CrosswordGrid({ data, puzzleId }: CrosswordGridProps) {
  const [cells, setCells] = useState<Record<string, string>>({});
  const firstEntry = data.entries.find((e) => e.dir === "ACROSS") ?? data.entries[0];
  const [activeKey, setActiveKey] = useState<string | null>(
    firstEntry ? key(firstEntry.number, firstEntry.dir) : null,
  );
  const [activeCell, setActiveCell] = useState<[number, number] | null>(
    firstEntry ? [firstEntry.row, firstEntry.col] : null,
  );
